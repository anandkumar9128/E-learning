  ...
  return (
    <div className="flex items-center justify-center h-[80vh] bg-white">
        <div className="bg-white p-[20px] border border-[#ddd] shadow-md rounded-[10px] text-center w-[300px]">
            <h2 className='text-[24px] text-[#8a4baf] mb-[15px] font-semibold'>Register</h2>
            <form onSubmit={submitHandler} action="" className='text-left'>
                <label htmlFor="name" className='block mb-[5px] text-[14px] text-[#333] '>Name<span className='text-red-500'> *</span></label>
                <input type="text" value={name} onChange={(e)=>setName(e.target.value)} required className='w-full p-[10px] border border-[#ddd] rounded-[4px] mb-[15px]'/>
                ...
                <button type="submit" disabled={btnLoading} className='w-full px-[20px] py-[10px] bg-[#8a4baf] text-white border-none rounded-[4px] cursor-pointer hover:bg-[#5f3575] transition-all duration-300'>{btnLoading ? "Please Wait..." : "Register"}</button>
            </form>
            <p className='mt-[15px] text-[14px] text-[#333]'>
                Already have an account? <Link to="/login" className='text-[#8a4baf] hover:underline'>Login</Link>
            </p>
        </div>
    </div>
  )
